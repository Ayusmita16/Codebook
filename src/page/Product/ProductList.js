...
import { getPdtList } from '../../services/ProductService';
...
export function ProductList() {
    ...
    useEffect(() => {
        async function fetchFeaturedPdts() {
            try {
                const data = await getPdtList(searchTerm);
                initialProductList(data)
            }
            catch (error) {
                toast.error(error.message, { closeButton: true, position: "bottom-centre" })
            }
        }
        fetchFeaturedPdts();
    }, [])//eslint-disable-line
